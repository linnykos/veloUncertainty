...
# glf_mark
def run_scv_and_plot_glf_mark(split_seed):
	...
	total_mark = sc.read(data_folder+'v4_'+dataset_long+'/glf_genes_mark/seed317/scv/adata_'+dataset_short+'_'+method+'_total_mark_genes.h5ad')
...
## glf_markctrl
def run_scv_and_plot_glf_markctrl(split_seed, grid_seed):
	...
	print_message_with_time('################## end velocity') 
	total_ctrl = sc.read(data_folder+'v4_'+dataset_long+'/glf_genes_mark/seed317/scv/adata_'+dataset_short+'_scv_total_markctrl_genes.h5ad')
	...
	print('############### All done for split seed'+str(split_seed)+', grid seed'+str(grid_seed))

"""
split_seeds = [320,323,326,329]
grid_seeds = [227,230,233,236]
for i in range(1):
	run_scv_and_plot_glf_mark(split_seeds[i])
	run_scv_and_plot_glf_markctrl(split_seeds[i], grid_seeds[i])
"""

split_seeds = [323,326,329]
grid_seeds = [230,233,236]
for i in range(3):
	run_scv_and_plot_glf_mark(split_seeds[i])
	run_scv_and_plot_glf_markctrl(split_seeds[i], grid_seeds[i])
