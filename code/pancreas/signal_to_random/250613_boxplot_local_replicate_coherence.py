...
def compute_df_cos_sim_corr_across_methods(method, dataset_long, dataset_short, type, data_folder, seeds=[317,320,323,326,329]):
    ...
    for i in range(len(seeds)):
        ...
        if ('u' in type): 
            ...
            df[method+'_replicate_coherence_'+str(seed)] = cos_sim
        ...
        if (not 'velocity_confidence' in total.obs.columns):
            ...
            df[method+'_local_coherence_'+str(seed)] = velo_conf
...
df.to_csv(data_folder+dataset_short+'_local_repl_coherence_317.csv')
...
df = pd.read_csv(data_folder+dataset_short+'_local_repl_coherence_317.csv')
