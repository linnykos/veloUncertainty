...
def run_countsplit_with_overdispersion(S,U,split_seed,overdisp_S,overdisp_U):
    ...
    s1, s2  = countsplit(S,overdisps=overdisp_S,epsilon=np.array([.3,.7]))
    u1, u2  = countsplit(U,overdisps=overdisp_U,epsilon=np.array([.3,.7]))
    return [[s1,u1],[s2,u2]]
...
adata_split1.write(data_folder+'v4_test/v4_'+dataset_long+'/seed'+str(split_seed)+'_'+dataset_short+'_allgenes_37-3.h5ad')
adata_split2.write(data_folder+'v4_test/v4_'+dataset_long+'/seed'+str(split_seed)+'_'+dataset_short+'_allgenes_37-7.h5ad')
