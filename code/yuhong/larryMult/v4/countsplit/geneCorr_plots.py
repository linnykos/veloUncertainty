...
split1_allgenes = sc.read_h5ad(data_folder+'v4_larryMult/seed317_larryMult_split1_allgenes.h5ad')
split2_allgenes = sc.read_h5ad(data_folder+'v4_larryMult/seed317_larryMult_split2_allgenes.h5ad')
...
total = read_raw_adata(dataset_long)
...
# Plotting spliced
axes[0].scatter(df['overdisps_S'], df['cor_spliced'], color='royalblue',alpha=0.4)
axes[0].set_title("Correlation between splits (seed=317, spliced), N="+str(np.sum(~np.isnan(df['overdisps_S']+df['cor_spliced']))))
axes[0].set_xlabel("overdispersion")
axes[0].set_ylabel("Correlation")
axes[1].scatter(df['overdisps_U'], df['cor_unspliced'],color='royalblue',alpha=0.4)
axes[1].set_title("Correlation between splits (seed=317, unspliced), N="+str(np.sum(~np.isnan(df['overdisps_U']+df['cor_unspliced']))))
...
# Adjusting layout to avoid overlap
plt.tight_layout()
plt.savefig(fig_folder+'corr_overdisps_allgenes_clip50.png') 
...
axes[0].set_title("Correlation between splits (seed=317, spliced), N="+str(len(df['cor_spliced'][~np.isnan(df['cor_spliced'])])))
axes[0].set_xlabel("fraction of nonzeros (Spliced)")
axes[0].set_ylabel("Correlation")
axes[1].scatter(df['frac_nnz_U'], df['cor_unspliced'], color='royalblue',alpha=0.4)
axes[1].set_title("Correlation between splits (seed=317,unspliced), N="+str(len(df['cor_unspliced'][~np.isnan(df['cor_unspliced'])])))
