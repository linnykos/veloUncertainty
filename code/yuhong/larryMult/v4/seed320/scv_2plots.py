...
c1,n1 = compute_cosine_similarity_intersect(split1,split2,method) #235
c2,n2 = compute_cosine_similarity_union(split1,split2,method)#551
np.quantile(c1,[0.,.25,.5,.75,1.]) # array([-0.44579732,  0.2510362 ,  0.39312359,  0.53679069,  0.81544205])
np.quantile(c2,[0.,.25,.5,.75,1.]) # array([-0.39120203,  0.22224183,  0.34311741,  0.46872054,  0.75203056])
...
np.corrcoef(split1.obs['velocity_confidence'],split2.obs['velocity_confidence']) # -0.11272204
...
np.round(np.mean(v2s_mean),4) # 0.2229
np.round(np.mean(v2s_median),4) # 0.233

np.round(np.var(v2s_mean),4) # 
np.round(np.var(v2s_median),4) 

# paired: 0.33 0.3431
# shuffled: 0.2229 0.233
