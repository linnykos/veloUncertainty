...
for i in range(5):
    ...
    if 'scv' in method:
        ...
        path_split1 = data_folder+'seed'+str(split_seed)+'/'+method+'/'+ [s for s in adata_names if re.search('split1_*outputAdded', s)][0]
        path_split2 = data_folder+'seed'+str(split_seed)+'/'+method+'/'+ [s for s in adata_names if re.search('split2_*outputAdded', s)][0]
...
df_cos_sim = pd.read_csv(data_folder+'cos_sim_5seeds_'+dataset_short+'_'+gene_set_name+'.csv')
